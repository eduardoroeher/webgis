...
// @version      1.11
// @description  Preenche automaticamente campos no WebGIS - implementação BUSCA CHAVE NO GOOGLE DRIVE
...
=============================*/
    ...
    async function buscarChave(valorChave) {
        // Aplica trim na chave antes de utilizá-la
        const chaveTratada = valorChave ? valorChave.trim() : "";

        // URL do Apps Script
        const url = `https://script.google.com/macros/s/AKfycbyjeoUpc78NPSlqpgHyQGuZS3y8X5ezmJuDE7kyfrT3mrmw1sxlzYfuiD9EWCiZ1Kw/exec?chave=${encodeURIComponent(chaveTratada)}`;

        console.log("Buscando chave:", chaveTratada); // Log da chave que está sendo buscada
        ...
        try {
            ...
            if (dados.retornoDaSaida && dados.retornoDaSaida.length > 0) {
                // Exibe os dados correspondentes da chave
                const info = dados.retornoDaSaida[0]; // Pega a primeira ocorrência
            ...
            `);
                ...
                console.warn("Nenhum dado encontrado para a chave:", chaveTratada);
                exibirNotificacao("Nenhum dado encontrado para a chave: " + chaveTratada);
            }
        } catch (error) {

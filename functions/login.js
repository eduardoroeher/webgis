...
async function frmLogin(versao) {
    ...
    if (popupExistente) {
        ...
        let popupContent = `
            <p style="font-size: 12px; color: gray; text-align: left; margin-bottom: 10px;">Versão: ${versao}</p>
        `;
        ...
        if (currentUrl.includes('id=c52271fd9c1e4515a1f1dafd8ce5ad2a')) {
            popupContent += `
                ...
                <select id="tipoCadastroSelect" style="width: 100%; padding: 10px; margin-bottom: 20px;">
                    ...
                    <option value="${TipoCadastro.BIC_BLOCOS_CQ}">BIC BLOCOS CQ</option>
                </select>
            `;
        ...
        return new Promise((resolve, reject) => {
            document.getElementById('confirmarTipoCadastro').addEventListener('click', () => {
                ...
                resolve({ nome, tipoCadastro }); //Retorna um objeto
            });

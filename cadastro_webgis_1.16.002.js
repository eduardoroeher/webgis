...
(function() {
	...
	async function preencherCampos() {
		if (tipoCadastro === TipoCadastro.NOVO_CADASTRO) {
			...
			await esperar(2000); // Aguardar 1 segundo entre preenchimentos
			preencherRevisao(nomeCadastrador);
			await esperar(2000);
			preencherDataModificacao();
		} 
		...
		return new Promise((resolve) => {
			...
			if (currentUrl.includes('id=c52271fd9c1e4515a1f1dafd8ce5ad2a')) {
				...
				bicLabel = `
					...
					<p style="text-align: center; font-size: 12px; color: #666;">
						Para ativar ou desativar o modo automático, pressione Ctrl + M.
					</p>`;
			}
...
//----------------------------------------------
	...
	exibirNotificacao(`MODO AUTOMÁTICO DESATIVADO! Presisone Alt + a para ativar ou desativar.`);

...
// @version      1.16
// @description  implementação BIC e início de alterações nos botões de atalho
...
(function() {
	...
	async function preencherCampos() {
		if (tipoCadastro === TipoCadastro.NOVO_CADASTRO) {
		...
		else if (tipoCadastro === TipoCadastro.BIC_REVISAO) {
			console.log("ACESSANDO BIC:", tipoCadastro);
			...
			await esperar(500); // Aguardar 1/2 segundo entre preenchimentos
	...
	function exibirComboboxTipoCadastro() {
		...
		if (currentUrl.includes('id=c52271fd9c1e4515a1f1dafd8ce5ad2a')) {
			...
			bicLabel = `
				...
				<p style="text-align: center; font-size: 12px; color: #666;">
					Para ativar ou desativar o modo automático, pressione Ctrl + M. Caso seja ativado, não será mais necessário clicar no "Editor Inteligente" ou utilizar a função "Ctrl + botão do meio do mouse"
				</p>
			`;
		}
		...
		if(idFase === 1){
			...
			container.innerHTML = `
				...
				<select id="tipoCadastroSelect" style="height: 40px; font-size: 14px;">
					<option value="${TipoCadastro.BIC_REVISAO}">BIC Revisão</option>
				</select>
...
//----------------------------------------------
	...
	// Alterna o modo automático ao pressionar Ctrl + M e exibe notificação
	function configurarAtalhosTeclado() {
		...
		document.addEventListener('keydown', function(event) {
			...
			if (event.ctrlKey && event.key.toLowerCase() === 'm') {
				...
				exibirNotificacao(`Modo automático ${status}: ` + versionamento);
				console.log(`Modo automático ${status}: ` + versionamento);
			}


			if (event.altKey && event.key.toLowerCase() === 'c') { // Verifica se Alt + C foi pressionado
				event.preventDefault(); // Impede comportamentos padrão, se necessário
				...
				if (!nomeCadastrador) {
					...
					exibirNotificacao(`Modo automático ativado! \nPressione Alt + C para preencher os campos!\nVersão: ` + versionamento);
				}
	...
	function adicionarOuvinteBotaoMouse() {
		document.addEventListener('mousedown', function(event) {
			...
			else if (event.button === 1 && event.ctrlKey && tipoCadastro === TipoCadastro.REVISAR_CHAVES) { // Botão do meio com Ctrl
			...
			// Modo automático para carregar informações na tela clicando apenas no ponto.
				...
				console.log("Círculo clicado com o botão esquerdo!");
				console.log(`Cor de preenchimento: ${fillColor}`);
				console.log(`Cor da borda: ${strokeColor}`);
				console.log(`Posição: (${cx}, ${cy})`);

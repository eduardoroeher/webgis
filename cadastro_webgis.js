...
(function() {
    ...
    console.log("Script carregado! 1.10");
...
function preencherCampos() {
    if (tipoCadastro === TipoCadastro.NOVO_CADASTRO) {
		...
		// CAMPO STATUS
		const promises = listaInputsRevisarChaves.map(({ selector, valor }) => {
			return new Promise((resolve) => {
				esperarCarregamentoCombobox(selector, valor, (comboboxInput, valor) => {
					abrirCombobox(comboboxInput, valor);
					resolve(); // Resolve a Promise quando o combobox for carregado e o valor for selecionado
				});
			});
		});

		// Espera todas as Promises serem resolvidas antes de continuar
		Promise.all(promises).then(() => {
			console.log("Todos os comboboxes foram preenchidos.");

			// Só continua após os comboboxes terem sido processados
			preencherRevisao(nomeCadastrador);
			preencherDataModificacao();
		});
	}
